...
export const movieRepository = {
  ...
  async create(payload: MovieRecord) {
    ...
    const created = await MovieModel.create({
      ...payload,
      generi: payload.generi.map((id) =>
        parseObjectId(id, "genereId"),
      ),
      registi: payload.registi.map((id) =>
        parseObjectId(id, "registaId"),
      ),
      attori: payload.attori.map((id) =>
        parseObjectId(id, "attoreId"),
      ),
    });
  ...
  async updateById(id: string, payload: MovieRecord) {
    ...
    return MovieModel.findByIdAndUpdate(
      ...
      {
        ...payload,
        generi: payload.generi.map((ref) =>
          parseObjectId(ref, "genereId"),
        ),
        registi: payload.registi.map((ref) =>
          parseObjectId(ref, "registaId"),
        ),
        attori: payload.attori.map((ref) =>
          parseObjectId(ref, "attoreId"),
        ),
      },
