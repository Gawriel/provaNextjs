...
function asIdList(value: unknown, label: string): string[] {
  ...
  return value.map((id, index) => {
    if (typeof id !== "string") {
      throw new ApiError(
        400,
        `${label}[${index}] deve essere una stringa.`,
      );
    }
...
function parseMovie(body: unknown): MovieRecord {
  ...
  if (!titolo) {
    throw new ApiError(400, "Titolo obbligatorio.");
  }
...
function movieFilter(
  ...
  if (genereId) {
    filter.generi = parseObjectId(genereId, "genereId");
  }
  ...
  if (anno) {
    ...
    if (!Number.isInteger(year)) {
      throw new ApiError(400, "Filtro anno non valido.");
    }
...
function parsePagination(searchParams: URLSearchParams) {
  ...
  if (!Number.isInteger(rawPage) || rawPage < 1) {
    throw new ApiError(400, "Pagina non valida.");
  }

  if (!Number.isInteger(rawLimit) || rawLimit < 1) {
    throw new ApiError(400, "Limite non valido.");
  }

  return {
    page: rawPage,
    limit: Math.min(rawLimit, MAX_PAGE_SIZE),
  };
}
...
export const movieService = {
  async getMovies(searchParams: URLSearchParams) {
    const filter = movieFilter(searchParams);
    const { page, limit } = parsePagination(searchParams);

    debugLog(4, "MovieService", "getMovies filter", {
      filter,
      page,
      limit,
    });

    const { rows, total } = await movieRepository.findAll(
      filter,
      page,
      limit,
    );

    const movies = serializeDoc(rows) as MovieDto[];
  ...
  async getMovie(id: string): Promise<MovieDto> {
    ...
    if (!row) {
      throw new ApiError(404, "Film non trovato.");
    }
  ...
  async createMovie(body: unknown): Promise<MovieDto> {
    ...
    debugLog(3, "MovieService", "createMovie", {
      id,
